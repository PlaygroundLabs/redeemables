...
contract Certificates is ERC1155ShipyardRedeemableMintable {
    ...
    function batchSetTrait(uint256[] memory tokenIds, bytes32 traitKey, bytes32[] memory traitValues)
        public
        onlyOwner
    {
        require(tokenIds.length == traitValues.length, "tokenIds and traitValues values must have the same length");
    ...
    function adminMint(address to, uint256 numTokens, uint256 amount) public onlyOwner {
